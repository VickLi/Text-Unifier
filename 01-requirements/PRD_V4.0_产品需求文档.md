...
| | D5 | Minimap | 右侧 30px 缩略图。将预览区**完整文本高度等比例压缩**至 Minimap 容器高度，每个色条代表压缩后的一段等分区域；单个色条视觉规格 = 2px 高 + 1px 间距（间距不参与映射）。悬停高亮预览，点击跳转到对应文本位置。连接点所在区域色条用特殊颜色标记（自动合并=黄，待确认=蓝） | V3.2 |
| | D6 | 悬停溯源 | 悬停文本任意位置 300ms → Tooltip 显示「来源：文件A」；连接点处显示「重叠区：文件A ↔ 文件B (N字重叠)」 | V3.2 |
| | D7 | Tooltip 跟随 | Tooltip 跟随鼠标 + 8px 偏移，不超出窗口边界 | V3.2 |
| | D8 | 只读展示 | 预览区文本以只读方式展示，不支持编辑 | V3.2 |
| | D9 | 底部状态 | 显示：总字符数 + 连接点数(自动N/待确认M) + 预估导出大小（UTF-8 字节数） | V2.0 |
| | D10 | 空状态 | 无文件时显示「拖拽 .txt 文件到此处或点击 + 按钮添加」 | 补充 |
| **J. 导出** | J1 | 导出格式 | 纯净 `.txt`，UTF-8 编码（不带 BOM），`\r\n` 换行 | V1.0 |
...
| D. 预览区 | 连续文本只读展示、连接标记、区域切断/连接、Minimap、悬停溯源 |
...
| 内容清洗 | E. 繁简转换、全角转换、搜索替换 | 移至独立项目「Text Unifier 内容工具」 |
| 撤回栈 | H. 5 步撤回、Ctrl+Z/Y | 移至独立项目「Text Unifier 内容工具」 |
| 文本编辑 | D. 预览区文本可编辑 | 预览区改为只读（无撤回栈保护） |
...
| AC-D06 | Minimap 全量覆盖 | 预览区文本高度远超 Minimap 容器高度 | 查看右侧 | Minimap 完整覆盖全文范围（色条从顶到底均匀分布），非仅显示开头部分 |
| AC-D07 | Minimap 跳转 | Minimap 显示 | 点击 Minimap 中间偏下位置 | 预览区滚动到全文约 50%~60% 位置 |
| AC-D08 | Minimap 连接点标记 | 合并后有 1 个自动合并连接点位于全文 30% 位置 | 查看 Minimap | Minimap 约 30% 位置处色条为黄色 |
| AC-D09 | 悬停溯源 | 文本来自 A.txt | 悬停文本 300ms | Tooltip:「来源：A.txt」；连接点处显示「重叠区：A ↔ B (N字重叠)」 |
| AC-D10 | 只读展示 | 预览区显示文本 | 点击或双击文本 | 文本不可编辑，光标不出现 |
| AC-D11 | 连接标记不可编辑 | 连接标记存在 | 尝试编辑标记 | 连接标记不可被编辑或删除 |
| AC-D12 | 底部状态 | 有 2 连接点(1自动1待确认) | 查看底部 | 「总 1,234 字 | 连接点 2 (自动1/待确认1) | 预估 1.2 KB」 |
| AC-D13 | 空状态 | 无文件 | 查看预览区 | 「拖拽 .txt 文件到此处或点击 + 按钮添加」 |
...
| AC-J02 | 预览一致 | 合并后有预览内容 | 导出 | 导出内容与预览区显示文本一致（排除连接标记） |
